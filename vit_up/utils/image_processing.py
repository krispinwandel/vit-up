...
def rotate_crop(img_orig_np, bbox, angle_deg):
    xmin, ymin, xmax, ymax = bbox.tolist()
    w, h = xmax - xmin, ymax - ymin
    s = max(w, h)
    diam = int(np.ceil(np.sqrt(2) * s))
    diam = diam + (diam % 2)  # make even
    assert s % 2 == 0, "bbox width and height must be even"
    pad = (diam - s) // 2  # keep parity
    img_orig_np_pad = np.pad(img_orig_np, ((pad, pad), (pad, pad), (0, 0)), mode="edge")
    xmin_pad = xmin
    ymin_pad = ymin
    xmax_pad = xmax + 2 * pad
    ymax_pad = ymax + 2 * pad
    img_crop_np = img_orig_np_pad[ymin_pad:ymax_pad, xmin_pad:xmax_pad]
    img_crop_torch = torch.from_numpy(img_crop_np).permute(2, 0, 1)
    img_crop_torch = TF.rotate(
        img_crop_torch[None],
        angle_deg,
        interpolation=TF.InterpolationMode.BILINEAR,
    )
    img_crop_torch = TF.center_crop(img_crop_torch, (h, w))[0]
    return img_crop_torch.numpy().transpose(1, 2, 0)
